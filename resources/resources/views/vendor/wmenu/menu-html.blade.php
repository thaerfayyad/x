...
$nav=\App\Models\Config::where('user_id', '=',$company_id)->where('key', '=', 'nav_menu')->first();
if($nav){
    if($nav->value)
    $nav_menu = \Harimayco\Menu\Models\Menus::findOrFail($nav->value);
    \Illuminate\Support\Facades\Config::set('nav_menu', $nav->value);
}
else{
                        ...
                        <div class="wrap">
                            ...
                            <div class="manage-menus">
                                <form method="get" action="{{ $currentUrl }}">
                                    ...
                                    {{-- <input type="hidden" name="company" class="company" value="1" /> --}}
                                    {{-- {!! Menu::select('menu', $menulist) !!} --}}
                                    <select name="menu" id="menu" class="menu_lists">
                                        <option value="0">Select menu</option>
                                        @foreach ($menu_list as $menu)
                                        <option value="{{$menu->id}}" @if(isset($nav_menu) && $nav_menu->id == request('menu')) selected @endif>{{$menu->name}}</option>
                                        @endforeach
                                    </select>
    ...
    <script>
        ...
        $(menu).each(function (key, value) {
            ...
            if ((value.id == "") || (value.id == 0)) {
                ...
                $('select#location_' + value.location).siblings('a').attr('href', "{{url('user/menu-manager')}}/{{ $company_id }}").text('Create New');
            } else {
                $('select#location_' + value.location).val(value.id).attr('selected', true).siblings('a').attr('href', "{{url('user/menu-manager')}}/{{ $company_id }}?menu=" + value.id)
            }
        ...
        $('#menu-locations-wrap').find('select').on('change', function () {
            if ($(this).val() == "") {
                ...
                $(this).siblings('a').attr('href', "{{url('user/menu-manager')}}/{{ $company_id }}").text('Create New');
            } else {
                $(this).siblings('a').attr('href', "{{url('user/menu-manager')}}/{{ $company_id }}?menu=" + $(this).val()).text('Edit')
