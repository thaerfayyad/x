        ...
        <div class="card-header">
            ...
            @can('course_create')
                <div class="float-right">
                    <a href="{{ route('admin.courses.create',['company' => $company]) }}"
            ...
            <div class="table-responsive">
                <div class="d-block">
                    <ul class="list-inline">
                        ...
                        |
                        <li class="list-inline-item">
                            <a href="{{ route('admin.courses.index',['company' => $company]) }}?show_deleted=1"
...
@push('after-scripts')
    <script>
        ...
        $(document).ready(function () {
            var route = '{{route('admin.courses.get_data')}}?company={{ $company }}';

            @if(request('show_deleted') == 1)
                route = '{{route('admin.courses.get_data',['show_deleted' => 1])}}&&company={{ $company }}';
            @endif

            @if(request('teacher_id') != "")
                route = '{{route('admin.courses.get_data',['teacher_id' => request('teacher_id')])}}&&company={{ $company }}';
            @endif

            @if(request('cat_id') != "")
                route = '{{route('admin.courses.get_data',['cat_id' => request('cat_id')])}}&&company={{ $company }}';
            @endif
